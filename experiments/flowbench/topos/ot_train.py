...
from topos.utils import UnitGaussianNormalizer,LpLoss, count_model_params, DictDataset, DictDatasetWithConstant
...
data_path = f'/media/HDD/mamta_backup/datasets/topos/flowbench/LDC_NS_2D/{resolution}x{resolution}/ot-data/LDC_NS_2D_boundary_{resolution}_{group_name}_{args.latent_shape}_expand{expand_factor}_reg1e-6_combined.pt'
data = torch.load(data_path)
origin_data = np.load('/media/HDD/mamta_backup/datasets/topos/flowbench/LDC_NS_2D/' + str(resolution) + 'x' + str(resolution) + '/' + group_name + '_lid_driven_cavity_X.npz')
print(data_path)
