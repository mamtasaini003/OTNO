...
class IPHI(nn.Module):
    def __init__(self, width=32):
        ...
        self.fc_code = nn.Linear(3, self.width)
...
file_name = f'results/geofno_boundary_{resolution}_{group_name}_3fields.csv'
################################################################
...
grid = torch.stack([grid_x, grid_y]).reshape(2,-1).T
...
input_xy, input_s, input_sdf, input_rr = [], [], [], []
for i in range(1000):
    out = output_data[i,0:3,:,:]
    input = input_data[i,:,:,:]
    sdf = input[1,:,:]
    out_flattened = torch.from_numpy(out).to(dtype=torch.float32).reshape(3,-1).T
    input_flattened = torch.from_numpy(input).to(dtype=torch.float32).reshape(3,-1).T
    ...
    input_rr.append(input_flattened[indices])
...
train_data = {"xy": train_xy, "s": train_s, "rr": train_rr}
test_data = {"xy": test_xy, "s": test_s, "rr": test_rr}
...
################################################################
model = FNO2d(modes, modes, width, in_channels=2, out_channels=3).cuda()
...
for ep in range(epochs):
    ...
    for data in train_loader:
        sigma, mesh, rr = data["s"].cuda(), data["xy"].cuda(), data["rr"].cuda() #torch.Size([1, 48381, 3]) torch.Size([1, 48381, 2]) torch.Size([1, 48381, 3])
        samples_x = torch.rand(batch_size, N_sample, 2).cuda() * 3 -1

        optimizer.zero_grad()
        out = model(mesh, iphi=model_iphi, code=rr)
    ...
    with torch.no_grad():
        for data in test_loader:
            sigma, mesh, rr = data["s"].cuda(), data["xy"].cuda(), data["rr"].cuda()
            out = model(mesh, iphi=model_iphi, code=rr)
