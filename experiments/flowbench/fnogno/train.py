...
file_name = f'results/training_results_{resolution}x{resolution}_{group_name}_{args.predict}_latent_{latent_res}.csv'
...
for i in range(1000):
    out = output_data[i,:,:]
    sdf = sdf_data[i,:,:]
    out_flattened = torch.from_numpy(out).to(dtype=torch.float32).flatten()
    sdf_flattened = torch.from_numpy(sdf).to(dtype=torch.float32).flatten()
    indices = torch.nonzero(sdf_flattened >= 0).squeeze()
    ...
    input_sdf.append(torch.from_numpy(sdf_latent).to(dtype=torch.float32))
