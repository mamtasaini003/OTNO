...
### Stage 1. **OT Encoder (Optimal Transport Map)**: A preprocessing module (`OT3Dto2DMapper`) that calculates a diffeomorphic projection $\mathcal{T}$ mapping the physical point cloud to a mathematically perfect uniform grid embedded in 3D (such as a Sphere or Torus depending on the genus). This acts as a dimensional reduction step (e.g., arbitrarily complex 3D boundary shells $\to$ 2D Latent Grids).
